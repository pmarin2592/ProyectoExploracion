...
class PcaDatos:
    ...
    def biplot(self):
        ...
        for i, variable in enumerate(self._eda.eda.numericas):
            ...
            fig.add_trace(go.Scatter(
                ...
                name=variable,
                showlegend=True,
                hovertemplate=f'{variable}<br>PC1: {self.pca.components_[0, i]:.3f}<br>PC2: {self.pca.components_[1, i]:.3f}<extra></extra>'
            ))
        ...
        fig.update_layout(
            title=f'Biplot Interactivo - PC1: {self.varianza_explicada[0] * 100:.1f}% | PC2: {self.varianza_explicada[1] * 100:.1f}%',
    ...
    def graficar_3d(self):
        ...
        hover_text = [
            f'Punto {i}<br>PC1: {self.resultado_pca[i, 0]:.3f}<br>PC2: {self.resultado_pca[i, 1]:.3f}<br>PC3: {self.resultado_pca[i, 2]:.3f}'
        ...
        fig.update_layout(
            title=f'PCA 3D Interactivo<br>PC1: {self.varianza_explicada[0] * 100:.1f}% | PC2: {self.varianza_explicada[1] * 100:.1f}% | PC3: {self.varianza_explicada[2] * 100:.1f}%',
            scene=dict(
                xaxis_title=f'PC1 ({self.varianza_explicada[0] * 100:.1f}%)',
                yaxis_title=f'PC2 ({self.varianza_explicada[1] * 100:.1f}%)',
    ...
    def graficar_3d_con_planos(self):
        ...
        fig.update_layout(
            title=f'PCA 3D con Planos de Proyección<br>PC1: {self.varianza_explicada[0] * 100:.1f}% | PC2: {self.varianza_explicada[1] * 100:.1f}% | PC3: {self.varianza_explicada[2] * 100:.1f}%',
            scene=dict(
                xaxis_title=f'PC1 ({self.varianza_explicada[0] * 100:.1f}%)',
                yaxis_title=f'PC2 ({self.varianza_explicada[1] * 100:.1f}%)',
                zaxis_title=f'PC3 ({self.varianza_explicada[2] * 100:.1f}%)',
    ...
    def graficar_analisis_cuadrantes(self):
        ...
        for i in range(len(self.resultado_pca)):
            ...
            if pc1 >= 0 and pc2 >= 0:
                cuadrantes_data['Q I'].append((pc1, pc2, i))
            elif pc1 < 0 and pc2 >= 0:
                cuadrantes_data['Q II'].append((pc1, pc2, i))
            elif pc1 < 0 and pc2 < 0:
        ...
        for q, color in colors.items():
            if cuadrantes_data[q]:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=q,
                    hovertemplate=f'{q} - Punto %{{customdata}}<br>PC1: %{{x:.3f}}<br>PC2: %{{y:.3f}}<extra></extra>',
                    customdata=indices
                ))
